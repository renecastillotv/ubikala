...
const $$Search = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${renderComponent($$result, "Layout", $$Layout, { "title": `Search Results${query ? ` for "${query}"` : ""} | PropiedadEnRD.com`, "description": "Search properties in the Dominican Republic. Houses, apartments, villas and land for sale and rent.", "keywords": "search properties dr, find real estate dominican republic", "alternateLanguages": alternateUrls }, { "default": ($$result2) => renderTemplate`  ${maybeRenderHead()}<section class="py-8 bg-white shadow-sm sticky top-16 z-40"> <div class="container-custom"> ${renderComponent($$result2, "SearchBox", $$SearchBox, { "variant": "inline" })} </div> </section>  <section class="section"> <div class="container-custom"> <div class="flex items-center justify-between mb-8"> <div> <h1 class="text-2xl font-bold text-gray-900"> ${query ? `Results for "${query}"` : "All Properties"} </h1> <p class="text-gray-600 mt-1"> <span class="font-semibold text-gray-900">${filteredProperties.length}</span> ${t.filters.results} </p> </div> <select class="select py-2 text-sm w-auto"> <option value="recent">Most Recent</option> <option value="price-asc">Price: Low to High</option> <option value="price-desc">Price: High to Low</option> <option value="area">Largest Area</option> </select> </div> ${filteredProperties.length > 0 ? renderTemplate`<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8"> ${filteredProperties.map((property) => renderTemplate`${renderComponent($$result2, "PropertyCard", $$PropertyCard, { "property": property })}`)} </div>` : renderTemplate`<div class="text-center py-16"> <div class="w-24 h-24 bg-gray-100 rounded-full flex items-center justify-center mx-auto mb-6"> <svg class="w-12 h-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"> <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path> </svg> </div> <h2 class="text-xl font-semibold text-gray-900 mb-2">${t.common.noResults}</h2> <p class="text-gray-600 mb-6">
...
}, "D:/portal PropiedadEnRD.com/src/pages/en/search.astro", void 0);

const $$file = "D:/portal PropiedadEnRD.com/src/pages/en/search.astro";
const $$url = "/en/search";
